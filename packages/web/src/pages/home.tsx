          ...
          <form onSubmit={handleCreatePost}>
            <div className="flex space-x-3">
              ...
              <div className="flex-1">
                ...
                {suggestions.length>0 && (<div className="mt-2 flex flex-wrap gap-1.5"><span className="text-[10px] text-slate-400">AI:</span>{suggestions.map(tag=>(<button key={tag} type="button" onClick={()=>addHashtag(tag)} className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-brand-50 text-brand-600 border border-brand-200 hover:bg-brand-100 dark:bg-brand-900/30 dark:text-brand-400 dark:border-brand-800/50 transition-colors"><Icons.Hash className="w-3 h-3 mr-0.5"/>{tag}<span className="ml-1 text-brand-400">+</span></button>))}</div>)}
